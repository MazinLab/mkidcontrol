...
class Lakeshore625ControlForm(FlaskForm):
    ...
    control_mode = SelectField("Control Mode", choices=COMMANDS625['device-settings:ls625:control-mode'])
    quench_ramp_rate = FloatField("Quench Ramp Rate (A/s)", default=0.020, validators=[NumberRange(0.01, 10)])
    baud_rate = SelectField("Baud Rate", choices=COMMANDS625['device-settings:ls625:baud-rate'], render_kw={'disabled': True})
